...
namespace workshop192.ServiceLayer.Tests
{
    ...
    public class BasketServiceTests
    {
        ...
        public void initial()
        {
            ...
            p1 = new Product("dress", "clothing", 20, 5, 2, store1, session1);
            p2 = new Product("coat", "clothing", 100, 2, 0, store1, session1);
            p3 = new Product("hat", "clothing", 200, 3, 10, store1, session1);
            p4 = new Product("iPhone XS", "Technology", 120, 1, 2, store2, session1);
            p5 = new Product("galaxy X", "Technology", 110, 4, 1, store2, session1);
        ...
        public void watchShoppingBasket()
        {
            ...
            Product p6 = new Product("dress", "clothing", 20, 5, 1, store3, session2);
            Product p7 = new Product("coat", "clothing", 100, 2, 2, store3, session2);
            Product p8 = new Product("hat", "clothing", 200, 3, 3, store3, session2);
            Product p9 = new Product("scooter", "vehicle", 120, 1, 7, store4, session2);
            Product p10 = new Product("Bicycle", "vehicle", 110, 4, 9, store4, session2);
            string s1 = basketService.addToCart(session2, store3, p6, 1);
            string s2 = basketService.addToCart(session2, store3, p7, 2);
            string s3 = basketService.addToCart(session2, store3, p6, 2); 
            string s4 = basketService.addToCart(session2, store3, p6, 3); 
            string s5 = basketService.addToCart(session2, store4, p6, 5); 
            string s6 = basketService.addToCart(session2, store4, p6, 4);
