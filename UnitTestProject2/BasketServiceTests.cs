...
namespace workshop192.ServiceLayer.Tests
{
    ...
    public class BasketServiceTests
    {
        ...
        public void addToCartTestSuccess()
        {
            try
            {
                basketService.addToCart(session1, store1, p1, 1); 
                basketService.addToCart(session1, store1, p2, 2); 
        ...
        public void addToCartTestFail()
        {
            try
            {
                basketService.addToCart(session1, store2, p3, 6); // should fail - too much 
            }
            catch(Exception e)
            {
                Assert.isTrue(true);
        ...
        public void watchShoppingBasketSuccess()
        {
            try
            {
                basketService.addToCart(session2, store3, p4, 2);
    ...
    public void watchShoppingBasketFail()
    {
        try
        {
            basketService.addToCart(session2, store3, p5, 2);
        ...
        {
                basketService.addToCart(session, store4, p6, 1);
                basketService.addToCart(session, store4, p7, 3);
                basketService.addToCart(session, store4, p8, 10);
                
                int sc1 = basketService.getCart(session3, store5); // shopping cart
                int amount = basketService.getCartAmount(sc1);

                basketService.removeFromCart(session3, store5, p6);
                
                Assert.AreNotEqual(amount, basketService.getTotalAmount(sc1));

                int newAmount = amount - basketService.getActualPrice(p6);

                Assert.AreEqual(newAmount, basketService.getTotalAmount(sc1));
